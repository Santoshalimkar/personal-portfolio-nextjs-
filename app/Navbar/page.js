...
const {toggle,setToggle}=ToggleSidebar()
  ...
  const style={
    ...
    activenav:"text-red-500 hover:text-red-500 visited:text-red-500 visited:no-underline uppercase hover:no-underline   cursor-pointer   font-semibold text-[0.8rem]"
  }
  return (
    <>
<div className={toggle?`sticky top-0 brightness-50 blur-sm`:'sticky top-0  z-20   '}>
      <div className='h-16 w-full  bg-[#ecf0f3] border-2 flex justify-between items-center z-10 '>
        ...
        <div>
            ...
            <Link href="/"  className={pathname === '/' ? `${style.activenav}` : `${style.navlink}`}>Home
            </Link>
            <Link href="/About"  className={pathname === '/About' ? `${style.activenav}` : `${style.navlink}`}>REsume
            </Link>
            <Link href="/Project"  className={pathname === '/Project' ? `${style.activenav}` : `${style.navlink}`}>project
            </Link>
            <Link href="/Contact"  className={pathname === '/Contact' ? `${style.activenav}` : `${style.navlink}`}>contact
            </Link>
            </ul>
            <div className='flex items-center lg:hidden md:hidden sm:hidden p-3 text-2xl text-[#ff014f]'><Menuicon  toggle={toggle} setToggle={setToggle}/></div>
        </div>
      </div>
    </div>
          {toggle && 
          <motion.div
                 ...
                 animate={{ x: '50%', opacity: [0, 0.3, 1], transition: { duration: 0.5, times: [0, 0.5, 1]} }}  // Animate to slide in from the right up to 20% from the left with fast start and slow end
               ...
               className='zindex bg-[#ecf0f3] w-4/5 z-20  top-0 h-screen fixed overflow-y-auto overflow-hidden shadow-md'>
                <div className='absolute  w-full flex justify-start p-2 items-end text-[#ff014f] '>
                  ...
                  <div className='zindex dark:bg-black'>
